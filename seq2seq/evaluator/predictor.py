...
class Predictor(object):
    ...
    def __init__(self, model, tgt_word2idx):
        """
        Predictor class to evaluate for a given model.
        Args:
            model (seq2seq.models): trained model. This can be loaded from a checkpoint
                using `seq2seq.util.checkpoint.load`
            tgt_word2idx (list): target vocabulary
        """
    ...
    def get_decoder_features(self, src_seq):
        src_id_seq = torch.LongTensor(src_seq).view(1, -1)
        if torch.cuda.is_available():
    ...
    def predict(self, src_seq):
        ...
        tgt_seq = [self._tgt_idx2word[tok.item()] for tok in tgt_id_seq]
        return tgt_seq
    ...
    def predict_n(self, src_seq, n=1):
        ...
        for x in range(0, int(n)):
            ...
            tgt_seq = [self._tgt_idx2word[tok] for tok in tgt_id_seq]
